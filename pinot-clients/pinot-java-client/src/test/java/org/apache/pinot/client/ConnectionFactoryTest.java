...
public class ConnectionFactoryTest {
  ...
  public void testZkConnection() {
    ...
    DynamicBrokerSelector dynamicBrokerSelector = Mockito.spy(new DynamicBrokerSelector(givenZkServers) {
      ...
      public List<String> getBrokers() {
        return Arrays.asList(givenBrokerInfo);
      }
    });
    ...
    Assert.assertEquals(connection.getBrokerList(), Arrays.asList(givenBrokerInfo));
  }
  ...
  public void testPropertiesConnection() {
    ...
    // Check that the broker list has the right length and has the same servers
    Assert.assertEquals(connection.getBrokerList(), Arrays.asList("127.0.0.1:1234", "localhost:2345"));
  }
  ...
  public void testBrokerList() {
    ...
    // Check that the broker list has the right length and has the same servers
    List<String> brokers = Arrays.asList(broker1, broker2);
  ...
  public void testBrokerListWithHeaders() {
    // Create the connection
    List<String> brokers = Arrays.asList("127.0.0.1:1234", "localhost:2345");

    Map<String, String> headers = Collections.singletonMap("Caller", "curl");
